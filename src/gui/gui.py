...
class App(tk.Tk):
    ...
    def __launch_dialog(self):
        ...
        # Textos
        self.title1 = tk.Label(self.first_panel, text="ROBOT VISION SYSTEM", font=(
            'Helvetica', 24, 'bold'), fg='#EA6749')
        self.title1.grid(row=0, column=0, columnspan=2, padx=5, pady=10)
        self.title2 = tk.Label(self.first_panel, text="STEREO VISION", font=(
            'Helvetica', 20, 'bold'), fg="#C42E0B")
        self.title2.grid(row=1, column=0, columnspan=2, padx=5, pady=10)

        self.texto1 = tk.Label(
            self.first_panel, text="Camera settings", font=16)
        ...
        self.combo_resolucion = ttk.Combobox(
            self.first_panel, state="readonly", values=LIST_RESOLUTIONS, font=12)
        ...
        self.combo_fps = ttk.Combobox(
            self.first_panel, state="readonly", values=LIST_FPS, font=12)
        ...
        self.combo_robot = ttk.Combobox(
            self.first_panel, state="readonly", values=LIST_NAME_ROBOTS, font=12)
        ...
        self.checkbox = tk.Checkbutton(self.first_panel, text="Record a video",
                                       variable=self.checkbox_value, command=self.checkbox_clicked, font=12, padx=0)
    ...
    def start_camera(self):
        ...
        # Textos
        self.title1 = tk.Label(self.center_panel, text="ROBOT VISION SYSTEM", font=(
            'Helvetica', 24, 'bold'), fg='#EA6749')
        self.title1.grid(row=0, column=0, columnspan=2, padx=5, pady=10)
        self.title2 = tk.Label(self.center_panel, text="STEREO VISION", font=(
            'Helvetica', 20, 'bold'), fg="#C42E0B")
        self.title2.grid(row=1, column=0, columnspan=2, padx=5, pady=10)

        self.texto1 = tk.Label(
            self.center_panel, text="Camera settings", font=16)
        ...
        if (int(self.vid.get(3)) >= self.screen_width or int(self.vid.get(4)) >= self.screen_height):
            self.scale_factor = min(self.screen_width / int(self.vid.get(3)),
                                    self.screen_height / int(self.vid.get(4))) * self.reduction_factor
            ...
            self.canvas = tk.Canvas(
                self.center_panel, width=final_width, height=final_height)
            self.canvas_2 = tk.Canvas(
                self.center_panel, width=final_width, height=final_height)
        else:
            self.canvas = tk.Canvas(
                self.center_panel, width=self.camera_width, height=camera_height)
            self.canvas_2 = tk.Canvas(
                self.center_panel, width=self.camera_width, height=camera_height)
    ...
    def snapshot(self):
        ret, frame_original = self.vid.read()
        if ret:
            # frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            frame, frame_left, frame_right = process(
                frame_original, self.camera_width)
            file_name = get_name(self.path_robot)

            print(f"Image captured and saved as {file_name}")
            cv2.imwrite(file_name + "_ORIGINAL.jpg", frame_original)
    ...
    def update_camera(self):
        ...
        if ret:
            ...
            self.canvas_2.create_image(
                0, 0, image=self.photo_left, anchor=tk.NW)
            ...
            if self.record_video:
                if self.video_writer_left is None or self.video_writer_right is None:
                    ...
                    self.video_writer_left = cv2.VideoWriter(
                        file_name + '_LEFT.avi', fourcc, self.path_fps, (self.camera_width, frame.shape[0]))
                    self.video_writer_right = cv2.VideoWriter(
                        file_name + '_RIGHT.avi', fourcc, self.path_fps, (self.camera_width, frame.shape[0]))
